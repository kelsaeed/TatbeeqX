...
|---|---|---|
| `record` | Every `insertRow` / `updateRow` / `deleteRow` against any custom entity | `entity` (code), `on` (subset of `["created","updated","deleted"]`), optional `filter` (formula expression evaluated against the new row — must return truthy) |
...
Deferred to v2 (called out so they're tracked): incoming webhooks (auth scheme TBD), page-button triggers (needs `PageBlock` config extension), HTTP-route triggers.
...
- **v1 (this commit set, 2026-05-02)** — schema + engine + 3 triggers + 6 actions + JSON-editor UI + tests + template portability.
- **v2 (deferred)** — visual builder, incoming-webhook trigger, page-button trigger, send_email action (after SMTP lands), if/else trees, retry policy per HTTP action.
- **v3 (deferred)** — async job runner, parallel branches, workflow-to-workflow chaining via shared queue.
